...
const TREE = function (rootSel) {
  ...
  function colour(css) {
    ...
    // both legacy rgb(r, g, b, a) and modern rgb(r g b / a) — the space-separated
    // form silently produced a one-element array and lost the colour entirely
    const parts = m[1].replace('/', ' ').split(/[\s,]+/).filter(Boolean);
    const p = parts.map(v => v.indexOf('%') >= 0 ? parseFloat(v) / 100 : parseFloat(v));
  ...
  function node(el) {
    ...
    // A flex container only becomes Figma auto-layout if its children actually sit
    // gap-to-gap. The build spaces several stacks with margins, which the DOM reports
    // as position but not as gap — replaying those as auto-layout collapses the rhythm.
    let flows = isFlex && !(runs.length && kids.length);
    if (flows && kids.length > 1) {
      const dir = s.flexDirection.indexOf('column') === 0 ? 'V' : 'H';
      const gap = dir === 'V'
        ? (parseFloat(s.rowGap) || parseFloat(s.gap) || 0)
        : (parseFloat(s.columnGap) || parseFloat(s.gap) || 0);
      if (s.justifyContent.indexOf('space-') !== 0) {
        for (let i = 1; i < kids.length; i++) {
          const a = kids[i - 1], b = kids[i];
          const prevEnd = dir === 'V' ? a.y + a.h : a.x + a.w;
          const start = dir === 'V' ? b.y : b.x;
          if (Math.abs(start - (prevEnd + gap)) > 1.5) { flows = false; break; }
        }
      }
    }
    if (flows) {
